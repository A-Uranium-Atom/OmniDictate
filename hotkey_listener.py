...
class HotkeyWorker(QObject):
    ...
    def __init__(
        self,
        ptt_key_str: str | None = None,
        is_capture_mode: bool = False,
        parent: QObject | None = None,
    ) -> None:
        """Initialize the HotkeyWorker thread."""
        ...
        if not self.is_capture_mode:
            self._parse_keys()
        print(
            f"Hotkey Listener Init: CaptureMode={self.is_capture_mode}, PTT='{self.ptt_key_str}'"
        )
    ...
    def _parse_keys(self) -> None:
        ...
        if self.ptt_key_str:
            try:
                ...
                error_msg = (
                    f"Error parsing PTT key '{self.ptt_key_str}': {e}. Using default."
                )
                print(error_msg)
    ...
    def key_to_string(self, key: keyboard.Key | keyboard.KeyCode) -> str:
        """Convert a keyboard key to its string representation."""
        if isinstance(key, keyboard.Key):
            return f"keyboard.Key.{key.name}"
        elif isinstance(key, keyboard.KeyCode):
            if key.char:
                if (
                    hasattr(key, "vk")
                    and key.vk is not None
                    and ((48 <= key.vk <= 57) or (65 <= key.vk <= 90))
                ):
                    return f"keyboard.KeyCode.from_char('{key.char}')"
                else:
                    return f"keyboard.KeyCode.from_char('{key.char}')"
            elif hasattr(key, "vk") and key.vk is not None:
                return f"keyboard.KeyCode(vk={key.vk})"
        return str(key)
    ...
    def start_listening(self) -> None:
        ...
        if self.listener_thread and self.listener_thread.is_alive():
            return
        print(f"Starting hotkey listener (Capture Mode: {self.is_capture_mode})...")
        ...
        try:
            ...
            self.listener_thread = threading.Thread(
                target=self.listener.run, daemon=True
            )
